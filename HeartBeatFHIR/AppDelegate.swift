...
//        getHeartRates()
    ...
    func getHeartRates() {
        healthKitManager?.readHeartRates() { (results, error) -> Void in
            ...
            if (error != nil) {
                print("Error: Reading HeartRate \(String(describing: error?.localizedDescription))")
                return
            } else {
    ...
    func getWeights() {
        healthKitManager?.readWeights() { (results, error) -> Void in
            NSLog("start HealthKitManager")
            if (error != nil) {
                print("Error: Reading Weight \(String(describing: error?.localizedDescription))")
                return
            } else {
                print("Weight read successfully!")
                weight = results as! [HKQuantitySample]
            }
        }
    }
